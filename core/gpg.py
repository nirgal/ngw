...
def get_instance():
    return GPG(gnupghome=GPG_HOME, options=['--no-auto-check-trustdb'],
               verbose=False)
...
class GpgLookupView(View):
    def get(self, request):
        if not GPG_HOME:
            return HttpResponse(
                'Keyring is disabled. Check your GPG_HOME settings',
                'text/html', 404)
        ...
        if not op:
            return HttpResponse(_build_content(
                title, 'pks request did not include a <b>op</b> property'),
                'text/html', 200)
        if not search:
            return HttpResponse(_build_content(
                title, 'pks request did not include a <b>search</b> property'),
                'text/html', 200)
        if search.startswith('-'):
            return HttpResponse(_build_content(
                title, 'pks request had an invalid <b>search</b> value'),
                'text/html', 403)
        if op == 'get':
            ...
            if not key:
                return HttpResponse(_build_content(
                    title, 'No matching keys in database'), 'text/html', 404)
            if 'mr' in options:
                return HttpResponse(key, 'application/pgp-keys', 200)
            return HttpResponse(_build_content(
                title, '<pre>'+key+'</pre>'), 'text/html', 200)

        return HttpResponse(_build_content(
            title, 'pks request had an invalid <b>op</b> property'),
            'text/html', 501)
